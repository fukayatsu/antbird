...
module Antbird
  module RestApi
    module RestApiV7_8
      ...
      # clear_scroll 
      # {"url" => "https://www.elastic.co/guide/en/elasticsearch/reference/master/search-request-body.html#_clear_scroll_api", "description" => "Explicitly clears the search context for a scroll."}
      ...
      def indices_create_data_stream(params = {})
        ...
        api_spec = @api_specs[api_name] ||= {"url" => {"paths" => [{"path" => "/_data_stream/{name}", "methods" => ["PUT"]}]}, "body" => {"required" => true}}
        request(api_name, api_spec, params, [:name])
      end
      ...
      def indices_flush_synced(params = {})
        ...
        api_spec = @api_specs[api_name] ||= {"url" => {"paths" => [{"path" => "/_flush/synced", "methods" => ["POST", "GET"]}, {"path" => "/{index}/_flush/synced", "methods" => ["POST", "GET"]}]}}
        request(api_name, api_spec, params, [:index])
      end
      ...
      def indices_get_data_streams(params = {})
        api_name = 'indices.get_data_streams'
        api_spec = @api_specs[api_name] ||= {"url" => {"paths" => [{"path" => "/_data_streams", "methods" => ["GET"]}, {"path" => "/_data_streams/{name}", "methods" => ["GET"]}]}}
        request(api_name, api_spec, params, [:name])
      end
